...
export const initializeFirebase = () => {
  try {
    // Allow explicitly disabling Firebase for certain environments
    if (process.env.FIREBASE_DISABLE === 'true') {
      console.warn('⚠️ Firebase Admin SDK disabled via FIREBASE_DISABLE=true');
      return;
    }
    ...
    if (admin.apps.length === 0) {
      ...
      if (saJson) {
        try {
          ...
          console.error('❌ Invalid FIREBASE_SERVICE_ACCOUNT_JSON. Must be valid JSON string. Skipping Firebase initialization.');
          return;
        }
      } else {
        ...
        if (missing.length) {
          console.warn(`⚠️ Firebase Admin credentials missing required env vars: ${missing.join(', ')}. Skipping Firebase initialization.`);
          return;
        }
      ...
      if (!projectId || typeof projectId !== 'string') {
        console.warn('⚠️ Service account missing valid project_id. Skipping Firebase initialization.');
        return;
      }
    ...
    console.error('❌ Firebase Admin SDK initialization failed (non-fatal):', error);
    // Do not exit; continue running without Firebase
  }
};
